...
describe('lib/blog/search.ts', () => {
  ...
  describe('getPostsByCategory', () => {
    ...
    it('handles case-insensitive category matching', () => {
      ...
      expect(posts).toHaveLength(2);
      expect(
        posts.every(post => post.category?.toLowerCase() === 'development')
      ).toBe(true);
    });
  ...
  describe('getRelatedPosts', () => {
    ...
    it('excludes the current post from results', () => {
      ...
      expect(relatedPosts.every(post => post.slug !== currentPost.slug)).toBe(
        true
      );
    });
    ...
    it('gives bonus score for same category', () => {
      ...
      const extendedPosts = [
        ...mockBlogPosts,
        postWithSameCategory,
        postWithDifferentCategory,
      ];
      mockGetPostSlugs.mockReturnValue([
        'post-1',
        'post-2',
    ...
    it('sorts by score then by date when scores are equal', () => {
      ...
      const jsRelatedPosts = relatedPosts.filter(
        p => p.slug === 'newer-post' || p.slug === 'older-post'
      );
